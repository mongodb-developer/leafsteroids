...
public class GhostHome : GhostBehavior
{
    ...
    private IEnumerator ExitTransition()
    {
        ...
        const float durationn = 0.5f;
        var elapsed = 0f;

        // Animate to the starting point
        while (elapsed < durationn)
        {
            Ghost.SetPosition(Vector3.Lerp(position, inside!.position, elapsed / durationn));
        ...
        while (elapsed < durationn)
        {
            Ghost.SetPosition(Vector3.Lerp(inside!.position, outside!.position, elapsed / durationn));
            elapsed += Time.deltaTime;
            yield return null;
        }
